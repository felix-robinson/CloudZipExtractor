...
/*        // ensure the host is a valid bucket name
        // Bucket names can consist of upper-case letters, lower-case letters, numbers, and "-". No other characters
        // are allowed. Even though your bucket can have upper-case letters, bucket names are case-insensitive. A
        // bucket name must be at least 6 characters long, and can be at most 50 characters long. These are all
        // allowed bucket names: myBucket, backblaze-images, and bucket-74358734. Bucket names that start with "b2-"
        // are reserved for Backblaze use.
        // check length
        if (host.length() < 6 || host.length() > 50)
            throw new IllegalArgumentException("B2FileSystemProvider must contain a host component that is between 6 and 50 letters long (inclusive)");
        if (host.startsWith("b2-") || host.startsWith("B2-"))
            throw new IllegalArgumentException("B2FileSystemProvider must not contain a host component that starts with b2- or B2-");
        Pattern pattern = Pattern.compile("[^abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-]", Pattern.LITERAL);
        Matcher matcher = pattern.matcher(host);
        if (matcher.find())
            throw new IllegalArgumentException("B2FileSystemProvider must only contain a host component composed from lower/upper case letters, numbers and -");*/

/**
 * B2FileSystem contains a single FileStore that is uniquely identified by the accountID.  Within the FileStore
 * there are up to 1000 RootDirectories (called buckets in B2 parlance).  Within a Root directory there are Files.
 */
public class B2FileSystem extends FileSystem {
    protected B2FileSystemProvider provider;
    protected String accountID;
    protected String accountKey;
    public static final String separator = "/"; // separating character between account and file names; note: the file name can also contain this character inside it.


    public B2FileSystem(B2FileSystemProvider b2FileSystemProvider, String accountID, String accountKey) {
        provider = b2FileSystemProvider;
    ...
    public Path getPath(String first, String... more) {
        // TODO double check this works for when B2FileSystemProvider calls it from its getPath() method
        // Should always be accountID, bucketName, (optional) fileName
        assert(first != null);
        assert(!first.isBlank());
        assert(more != null);
        assert(more.length > 0); // at least 1
        assert(more.length < 3); // no more than 2

        // TODO input validation
        // first != null and !isBlank()
        // if more != null then length == 1, more[0] != null and !isBlank()

        switch (more.length) {
            case 1: return new B2Path(this, first, more[0]);
            case 2: return new B2Path(this, first, more[0], more[1]);
        }
