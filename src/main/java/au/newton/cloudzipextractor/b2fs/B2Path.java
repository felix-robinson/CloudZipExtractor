...
public class B2Path implements Path, Cloneable {
    // TODO ensure B2Path support equals() so that B2FileSystemProvider isSameFile() works
    protected B2FileSystem b2FileSystem;
    protected String accountID;
    protected String bucketName;
    protected String fileName;

    public B2Path clone() {
        return new B2Path(b2FileSystem, accountID, bucketName, fileName);
    }

    /**
     * Provides this path as a String; from the root to the filename.
     * It is of the form {@code accountID separator bucketName separator fileName}
     * There isn't an additional separator character at the beginning or end, fileName may contain the separator
     * character as part of the name.
     *
     * @return this path as a String
     */
    protected String getCanonicalPath() {
        if (isFile()) return accountID + B2FileSystem.separator + bucketName + B2FileSystem.separator + fileName;
        if (isBucket()) return accountID + B2FileSystem.separator + bucketName;
        if (isAccount()) return accountID;
        throw new IllegalStateException("Path has no root");
    }
    ...
    public Boolean isAccount() {
        return (!hasFile() && !hasBucket() && hasAccount());
    }
    ...
    /**
     * create a root Path: B2 account only
     * @param fileSystem
     * @param accountID
     */
    public B2Path(B2FileSystem fileSystem, String accountID) {
        // TODO input validation
        b2FileSystem = fileSystem;
    ...
    /**
     * create a parent path: B2 account and bucket only
     * @param fileSystem
     * @param accountID
     * @param bucketName
     */
    public B2Path(B2FileSystem fileSystem, String accountID, String bucketName) {
        // TODO input validation
        b2FileSystem = fileSystem;
        this.accountID = accountID;
    ...
    /**
     * create a file path: B2 account, bucket and file
     * @param fileSystem
     * @param accountID
     * @param bucketName
     * @param fileName
     */
    public B2Path(B2FileSystem fileSystem, String accountID, String bucketName, String fileName) {
        // TODO input validation
        b2FileSystem = fileSystem;
        this.accountID = accountID;
        this.bucketName = bucketName;
    ...
    public boolean isAbsolute() {
        // TODO add functionality for relative path processing
        return true;
    }
    ...
    @Override
    public Path getRoot() {
        // root is the B2 account ID
        if (hasAccount()) return new B2Path(b2FileSystem, accountID);
        return null;
    }
    ...
    public Path getParent() {
        // only a file has a parent; that is the bucket.
        if (isFile()) return new B2Path(b2FileSystem, accountID, bucketName);
        return null;
    }
    ...
    public int getNameCount() {
        if (isFile()) return 2;
        if (isBucket()) return 1;
        return 0;
    }

    /**
     * Returns a name element of this path as a {@code Path} object.  B2File is always
     * canonical so the resulting Path contains all name elements from the root to
     * {@code index}.
     *
    ...
    public Path getName(int index) {
        if ((index < 0) || (index >= getNameCount())) throw new IllegalArgumentException("index out of bounds");
        // note: the getNameCount() check ensures that the next 2 lines can execute
        if (index == 0) return new B2Path(b2FileSystem, accountID, bucketName);
        if (index == 1) return new B2Path(b2FileSystem, accountID, bucketName, fileName);
        throw new IllegalArgumentException("should never get here");
    }

    /**
     * Returns a relative {@code Path} that is a subsequence of the name
     * elements of this path.  B2Path is always canonical so {@code beginIndex}
     ...
     * endIndex-1}.
     *
     * @param beginIndex the index of the first element, inclusive: must be set to 0
    ...
    public Path subpath(int beginIndex, int endIndex) {
        // gives a subpath from the bucket to the specified endIndex
        if (beginIndex != 0) throw new IllegalArgumentException("a begin index other than 0 is not supported");
        if (endIndex <= beginIndex) throw new IllegalArgumentException("endIndex is less than or equal to beginIndex");
        if ((endIndex - 1) >= getNameCount()) throw new IllegalArgumentException("end index out of bounds");

        return getName(endIndex - 1);
    }
    ...
    public boolean startsWith(Path other) {
        assert(other != null);
        // TODO fix this so that it compares each of the parts of the Path

        if (!(other instanceof B2Path b2Other)) return false;
        return this.getCanonicalPath().startsWith(b2Other.getCanonicalPath());
    }
     ...
     *
     * <p> {@code other} must be of the format provided by {@link #getCanonicalPath() getCanonicalPath()}
     *
    ...
    public boolean startsWith(String other) {
        ...
        return this.getCanonicalPath().startsWith(other);
    }

    /**
     * Tests if this path ends with the given path.  B2Path does not support
     * this functionality because it is always a canonical path.
    ...
    public boolean endsWith(Path other) {
        throw new UnsupportedOperationException("endsWith() is not supported by B2Path");
    }

    /**
     * Tests if this path ends with a {@code Path}.  B2Path does not support
     * this functionality because it is always a canonical path.
     * Throws an UnsupportedOperationException
     *
     * <p>Constructed by converting
     * the given path string, in exactly the manner specified by the {@link
    ...
    public Path normalize() {
        // B2File is always normalized
        return this.clone();
    }

    /**
     * Resolve the given path against this path.  Combines {@code other} with
     * this Path by adding any parts of the path that exist in {@code other}
     * that don't exist in this Path.
     *
     ...
     *
     * Returns a new Path that contains all the parts of this path and has any
     * additional parts from {@code other} added to it.
     *
    ...
    public Path resolve(Path other) throws IllegalArgumentException {
        // combine the 2 paths; throw IllegalArgumentException if other cannot be combined
        if (!(other instanceof B2Path b2Other)) throw new IllegalArgumentException("other must be a B2Path");

        if (this.isAccount()) {
            if (b2Other.isBucket()) return new B2Path(this.b2FileSystem, this.accountID, b2Other.bucketName);
            if (b2Other.isFile()) return new B2Path(this.b2FileSystem, this.accountID, b2Other.bucketName, b2Other.fileName);
        }
        if (this.isBucket()) {
            if (b2Other.isFile()) return new B2Path(this.b2FileSystem, this.accountID, b2Other.bucketName, b2Other.fileName);
        }
    ...
    public Path resolve(String other) {
        ...
        // other cannot contain the file system
        // must be in the form provided by getCanonicalPath
